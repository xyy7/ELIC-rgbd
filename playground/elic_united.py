...
class ELIC_united(CompressionModel):
    def __init__(self, config, **kwargs):
        ...
        # 这里改成了两倍，进行channel跨模态交互
        self.rgb_channel_context = nn.ModuleList(ChannelContextEX(in_dim=sum(slice_ch[:i]) * 2, out_dim=slice_ch[i] * 2, act=nn.ReLU) if i else None for i in range(slice_num))  # 因为是使用索引idx来跳过None

...
def parse_args(argv):
    ...
    parser.add_argument("-m", "--model", default="ckbd", help="Model architecture (default: %(default)s)")
    # parser.add_argument(
...
def main(argv):
    ...
    if args.experiment == "":
        args.experiment = f"sunrgbd_rgb_{args.model}_{args.quality}"
    ...
    train_dir = "/data/chenminghui/sunrgbd/train/train"
    val_dir = "/data/chenminghui/sunrgbd/train/val"

    train_dataset = sun(train_dir, True)
    test_dataset = sun(val_dir, False)
    device = "cuda"

    train_dataloader = DataLoader(train_dataset, batch_size=20, num_workers=8, shuffle=True, pin_memory=(device == "cuda"), drop_last=True)
    ...
    net = Cheng2020AnchorwithCheckerboard()
    if args.checkpoint != None:
        checkpoint = torch.load(args.checkpoint)
    ...
    if args.checkpoint != None:
        ...
        args.milestones = [start_epoch + int((args.epochs - start_epoch) * 0.75), start_epoch + int((args.epochs - start_epoch) * 0.9)]
        lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=args.milestones, gamma=0.1)

...
from utils_edge import nyuv2, sun
...
def compute_metrics(
    a: Union[np.array, Image.Image], b: Union[np.array, Image.Image], max_val: float = 1
) -> Tuple[float, float]:  # 会自动转换
        ...
        if current_step % 100 == 0:
            ...
            if out_criterion["ms_ssim_loss"] is not None:
                tb_logger.add_scalar(
                    "{}".format("[train]: ms_ssim_loss"), out_criterion["ms_ssim_loss"].item(), current_step
                )
...
def main(argv):
    ...
    if args.experiment == "":
        args.experiment = f"sunrgbd_depth_{args.model}_{args.quality}"
    ...
    train_dir = "/data/xyy/sunrgbd/train/train"
    val_dir = "/data/xyy/sunrgbd/train/val"
    train_dataset = sun(train_dir, True)
    test_dataset = sun(val_dir, False)
    device = "cuda"

    train_dataloader = DataLoader(
        train_dataset, batch_size=16, num_workers=8, shuffle=True, pin_memory=(device == "cuda"), drop_last=True
    )

    test_dataloader = DataLoader(test_dataset, batch_size=4, num_workers=1, pin_memory=(device == "cuda"))
    ...
    if args.checkpoint != None:
        ...
        args.milestones = [
            start_epoch + int((args.epochs - start_epoch) * 0.75),
            start_epoch + int((args.epochs - start_epoch) * 0.9),
        ]
        lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=args.milestones, gamma=0.1)
        ...
        if args.save:
            ...
            if (epoch + 1) % 200 == 0:
                save_checkpoint(
                    ...
                    # os.path.join('../experiments', args.experiment, 'checkpoints', "checkpoint_%03d.pth.tar" % (epoch + 1))
                    os.path.join(
                        "../experiments", args.experiment, "checkpoints", f"checkpoint_epoch{epoch}.pth.tar"
                    ),  # 存储空间不够
                )
            if is_best:

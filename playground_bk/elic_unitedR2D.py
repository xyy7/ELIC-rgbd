...
#         self.fusion = nn.Sequential(nn.Conv2d(in_dim, in_dim // 6, 1), act(), nn.Conv2d(in_dim // 6, out_dim * 4 // 3, 1), act(), nn.Conv2d(out_dim * 4 // 3, out_dim, 1))
...
class EntropyParametersPlus(nn.Module):
    def __init__(self, in_dim, out_dim, act=nn.GELU) -> None:
        super().__init__()
        self.fusion = nn.Sequential(nn.Conv2d(in_dim, in_dim // 4, 1), act(), nn.Conv2d(out_dim * 4 // 3, out_dim, kernel_size=5, stride=1, padding=2))
        self.se = SE_Block(in_dim)  # 切换成通道
...
class ELIC_united(CompressionModel):
    ...
    def entropy_estimate_index0(self, rgb_y_slice, rgb_hyper_params, depth_y_slice, depth_hyper_params):
        ...
        # rgb_Anchor
        rgb_params_anchor = self.rgb_entropy_parameters_anchor[0](torch.cat([rgb_hyper_params], dim=1))
        ...
        # rgb_Non-anchor
        # local_ctx: [B, H, W, 2 * C]
        rgb_params_nonanchor = self.rgb_entropy_parameters_nonanchor[0](torch.cat([rgb_local_ctx, rgb_hyper_params], dim=1))  # 这里其实也是可以考虑交互的
    ...
    def entropy_estimate_index1234(self, rgb_y_slice, depth_y_slice, rgb_hyper_params, depth_hyper_params, rgb_y_hat_slices, depth_y_hat_slices, idx):
        ...
        rgb_params_anchor = self.rgb_entropy_parameters_anchor[idx](torch.cat([rgb_channel_ctx, rgb_hyper_params], dim=1))  # 这里仍然可以添加另一个模态的信息
    ...
    def compress_index1234(
        ...
        # rgb-nonanchor
        # local_ctx: [B,2 * C, H, W]
        rgb_params_nonanchor = self.rgb_entropy_parameters_nonanchor[idx](torch.cat([rgb_local_ctx, rgb_channel_ctx, rgb_hyper_params], dim=1))  # 这里两个local是有问题的

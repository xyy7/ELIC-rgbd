...
from elic_unitedR2D import ELIC_united
...
class RateDistortionLoss(nn.Module):
    ...
    def get_final_loss(self, out):
        ...
        # if self.iter > math.inf:  # 先用mse训练一个epoch进行预热训练
...
def train_one_epoch(model, criterion, train_dataloader, optimizer, aux_optimizer, epoch, clip_max_norm, logger_train, tb_logger, current_step):
    ...
    for i, d in enumerate(train_dataloader):
        ...
        # summary(model, input_data=[rgb, depth])
        # exit()
        ...
        #         if name.find("rgb") != -1:
        #             rgb_grads[name] = param.grad.max()
        #             depth_name = name.replace("rgb", "depth")
        #             if name.replace("rgb", "depth") not in model.state_dict().keys():
        #                 print("no ", name.replace("rgb", "depth"),param.grad.max())
        #             else:
        #                 print("yes ", name.replace("rgb", "depth"),param.grad.max())

        #         else:
        #             depth_grads[name] = param.grad.max()
        #             rgb_name = name.replace("depth", "rgb")
        #             if rgb_name not in model.state_dict().keys():
        #                 print("no ", rgb_name)
        #             else:
        #                 print("yes ", rgb_name, param.grad.max())

        # # 输出梯度
        # # pprint(rgb_grads)
        # # pprint(depth_grads)
        # print(len(rgb_grads.keys()), len(depth_grads.keys()))
        if clip_max_norm > 0:
...
def parse_args(argv):
    ...
    parser.add_argument("--batch_size", type=int, default=4, help="Batch size (default: %(default)s)")  # 直接指定
    # parser.add_argument(
    #     "--test-batch-size",
    #     type=int,
    #     default=1,
    ...
    parser.add_argument("--clip_max_norm", default=2.0, type=float, help="gradient clipping max norm (default: %(default)s")
...
def main(argv):
    ...
    train_dir = "/data/xyy/nyu5k"
    val_dir = "/data/xyy/nyu5k/val"
    ...
    if args.checkpoint != None:
        ...
        if not args.start_epoch:
            ...
            # lr_scheduler.optimizer.step = args.start_epoch
            # lr_scheduler.optimizer._step_count = args.start_epoch

...
class EntropyParametersEX(nn.Module):
    def __init__(self, in_dim, out_dim, act=nn.GELU) -> None:
        ...
        self.se = MaxViTBlock(in_channels=out_dim, out_channels=out_dim, downscale=False, grid_window_size=(4, 4))
    ...
    def forward(self, params):
        ...
        """
        gaussian_params = self.se(self.fusion(params))

        return gaussian_params


class ELIC_united_CEE(CompressionModel):
    ...
    def entropy_estimate_index0(self, rgb_y_slice, rgb_hyper_params, depth_y_slice, depth_hyper_params):
        ...
        # rgb_Anchor
        rgb_params_anchor = self.rgb_entropy_parameters_anchor[0](
            torch.cat([rgb_hyper_params, depth_hyper_params], dim=1)
        )
        ...
        if self.quant == "ste":
            ...
            rgb_slice_anchor = self.rgb_gaussian_conditional.quantize(
                rgb_slice_anchor, "noise" if self.training else "dequantize"
            )
            rgb_slice_anchor = ckbd_anchor(rgb_slice_anchor)
        rgb_local_ctx = self.rgb_local_context[0](rgb_slice_anchor)

        # depth_Anchor
        depth_params_anchor = self.depth_entropy_parameters_anchor[0](
            torch.cat([depth_hyper_params, depth_hyper_params, rgb_local_ctx], dim=1)
        )
        ...
        if self.quant == "ste":
            ...
            depth_slice_anchor = self.depth_gaussian_conditional.quantize(
                depth_slice_anchor, "noise" if self.training else "dequantize"
            )
        ...
        # rgb_Non-anchor
        # local_ctx: [B, H, W, 2 * C]
        rgb_params_nonanchor = self.rgb_entropy_parameters_nonanchor[0](
            torch.cat([rgb_local_ctx, depth_local_ctx, rgb_hyper_params, depth_hyper_params], dim=1)
        )  # 这里其实也是可以考虑交互的
        ...
        if self.quant == "ste":
            ...
            rgb_slice_nonanchor = self.rgb_gaussian_conditional.quantize(
                rgb_slice_nonanchor, "noise" if self.training else "dequantize"
            )
        ...
        # depth_Non-anchor
        # local_ctx: [B, H, W, 2 * C]
        depth_params_nonanchor = self.depth_entropy_parameters_nonanchor[0](
            torch.cat([rgb_local_ctx, depth_local_ctx, rgb_hyper_params, depth_hyper_params], dim=1)
        )  # 这里其实也是可以考虑交互的
        ...
        if self.quant == "ste":
            ...
            depth_slice_nonanchor = self.depth_gaussian_conditional.quantize(
                depth_slice_nonanchor, "noise" if self.training else "dequantize"
            )
    ...
    def entropy_estimate_index1234(
        self,
        rgb_y_slice,
        depth_y_slice,
        rgb_hyper_params,
        ...
        rgb_channel_ctx = self.rgb_channel_context[idx](
            torch.cat(rgb_y_hat_slices, dim=1)
        )  # 如果是这样作为上下文，有可能出现把原来的信息给挤掉了，因为channel数量没有改变
        ...
        rgb_params_anchor = self.rgb_entropy_parameters_anchor[idx](
            torch.cat([rgb_channel_ctx, depth_channel_ctx, rgb_hyper_params, depth_hyper_params], dim=1)
        )  # 这里仍然可以添加另一个模态的信息
        ...
        depth_params_anchor = self.depth_entropy_parameters_anchor[idx](
            torch.cat([rgb_local_ctx, rgb_channel_ctx, depth_channel_ctx, rgb_hyper_params, depth_hyper_params], dim=1)
        )
        ...
        rgb_slice_nonanchor = self.get_nonanchor(
            rgb_params_nonanchor, rgb_slice_nonanchor, self.rgb_gaussian_conditional
        )
        rgb_y_slice_likelihoods = self.get_total_y_likelihood(
            rgb_params_anchor, rgb_params_nonanchor, rgb_y_slice, self.rgb_gaussian_conditional
        )
        rgb_y_hat_slice = rgb_slice_anchor + rgb_slice_nonanchor
        ...
        depth_slice_nonanchor = self.get_nonanchor(
            depth_params_nonanchor, depth_slice_nonanchor, self.depth_gaussian_conditional
        )
        depth_y_slice_likelihoods = self.get_total_y_likelihood(
            depth_params_anchor, depth_params_nonanchor, depth_y_slice, self.depth_gaussian_conditional
        )
    ...
    def entropy_estimate_united(self, rgb, depth, rgb_hyper_params, depth_hyper_params):
        ...
        for idx, (rgb_y_slice, depth_y_slice) in enumerate(zip(rgb_y_slices, depth_y_slices)):
            if idx == 0:
                ...
                ) = self.entropy_estimate_index0(rgb_y_slice, rgb_hyper_params, depth_y_slice, depth_hyper_params)
                ...
                (
                    ...
                    rgb_y_slice_likelihoods,
                    depth_y_slice_likelihoods,
                ) = self.entropy_estimate_index1234(
                    rgb_y_slice,
                    depth_y_slice,
                    rgb_hyper_params,
                    depth_hyper_params,
    ...
    def decompress(self, rgb_strings, depth_strings, shape):
        ...
        rgb_y_hat, depth_y_hat = self.bin2img_united(

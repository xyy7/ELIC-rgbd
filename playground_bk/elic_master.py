...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_ch: int, out_ch: int):
        ...
        self.leaky_relu = nn.ReLU(inplace=True)  # leakyReLU是在负数阶段，有一点点偏移于0
...
def deconv(in_channels, out_channels, kernel_size=5, stride=2):
    return nn.ConvTranspose2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, output_padding=stride - 1, padding=kernel_size // 2)  # 这是具有针对性的padding
...
class Channel_aligner(nn.Module):
    def __init__(self) -> None:
        ...
        self.conv1 = nn.Sequential(
            conv3x3(64, 256), nn.LeakyReLU(inplace=True), conv3x3(256, 256), nn.LeakyReLU(inplace=True), conv3x3(256, 256), nn.LeakyReLU(inplace=True), conv3x3(256, 256), nn.LeakyReLU(inplace=True)
        )
...
class ELIC_master(CompressionModel):
    def __init__(self, config, ch=3, **kwargs):
        ...
        self.local_context = nn.ModuleList(nn.Conv2d(in_channels=slice_ch[i], out_channels=slice_ch[i] * 2, kernel_size=5, stride=1, padding=2) for i in range(len(slice_ch)))
        self.channel_context = nn.ModuleList(ChannelContextEX(in_dim=sum(slice_ch[:i]), out_dim=slice_ch[i] * 2, act=nn.ReLU) if i else None for i in range(slice_num))
        ...
        for idx, y_slice in enumerate(y_slices):
            ...
            if idx == 0:
                ...
                if self.quant == "ste":
                    ...
                    slice_anchor = self.gaussian_conditional.quantize(slice_anchor, "noise" if self.training else "dequantize")
                ...
                if self.quant == "ste":
                    ...
                    slice_nonanchor = self.gaussian_conditional.quantize(slice_nonanchor, "noise" if self.training else "dequantize")

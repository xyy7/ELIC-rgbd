...
class HyperSynthesisEXcross_ESA(nn.Module):
    def __init__(self, M, N, act=nn.ReLU) -> None:
        super().__init__()
        self.M = M
        self.N = N
        self.rgb_reduction = nn.ModuleList([deconv(M, N), act(), bi_spf(N), deconv(2 * N, N), act(), bi_spf(N), conv(2 * N, N)])
        self.depth_reduction = nn.ModuleList([deconv(M, N), act(), nn.Identity(), deconv(2 * N, N), act(), nn.Identity(), conv(2 * N, N)])

...
class HyperSynthesisEXSingle(nn.Module):
    ...
    def __init__(self, N=192, M=192, act=nn.ReLU) -> None:
        super().__init__()
        self.M = M
        self.N = N
        ...
        self.depth_increase = nn.Sequential(
            ...
            nn.GELU(),
            SE_Block(2 * M),
            subpel_conv3x3(2 * M, M, 2),
            nn.GELU(),
            SE_Block(2 * M),
            conv3x3(2 * M, M * 3 // 2),
            nn.GELU(),
            SE_Block(M * 3),
            subpel_conv3x3(M * 3, M * 3 // 2, 2),
            nn.GELU(),
            conv3x3(M * 3 // 2, M * 2),
        )
    ...
    def forward(self, rgb, depth):
        rgb_hat = rgb
        depth_hat = depth
        for num, (rgb_bk, depth_bk) in enumerate(zip(self.rgb_increase, self.depth_increase)):
            if isinstance(depth_bk, SE_Block):
                depth_hat = depth_bk(torch.cat([depth_hat, rgb_hat], dim=1))
            else:
                rgb_hat = rgb_bk(rgb_hat)
                depth_hat = depth_bk(depth_hat)
...
class HyperSynthesisEXcrossPlus(nn.Module):
    def __init__(self, N, M, act=nn.ReLU) -> None:
        super().__init__()
        self.r_h_s1 = hyper_transform_block_plus(2 * N, M)
        self.r_h_s2 = hyper_transform_block_plus(2 * M, M * 3 // 2)
        self.r_h_s3 = hyper_transform_block_plus(M * 3, 2 * M, True)

        self.d_h_s1 = hyper_transform_block_plus(2 * N, M)
        self.d_h_s2 = hyper_transform_block_plus(2 * M, M * 3 // 2)
        self.d_h_s3 = hyper_transform_block_plus(M * 3, 2 * M, True)
...
class hyper_transform_block_plus(nn.Module):
    def __init__(self, in_channel, out_channel, is_last=False):
        super(hyper_transform_block_plus, self).__init__()
        down_in_ch = in_channel // 4
        self.down1 = conv1x1(in_channel, down_in_ch)

        self.se = MaxViTBlock(
            in_channels=down_in_ch, out_channels=down_in_ch, downscale=False, grid_window_size=(4, 4), num_heads=4
        )  # 需要比较注意输入的size，不建议，而且depth和rgb在低频区域的相似度会比较低。
        if is_last is False:
            self.deconv = deconv(down_in_ch, out_channel, stride=2, kernel_size=5)
            self.relu = nn.LeakyReLU(inplace=True)
        else:
            self.relu = None
            self.deconv = deconv(down_in_ch, out_channel, stride=1, kernel_size=3)  # united 发生了改变

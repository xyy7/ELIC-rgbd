...
from timm.models.layers import DropPath, Mlp, drop_path, trunc_normal_
...
def window_reverse(
    windows: torch.Tensor, original_size: Tuple[int, int], window_size: Tuple[int, int] = (7, 7)
) -> torch.Tensor:
...
def grid_reverse(
    grid: torch.Tensor, original_size: Tuple[int, int], grid_size: Tuple[int, int] = (7, 7)
) -> torch.Tensor:
...
class RelativeSelfAttention(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        qkv = (
            self.qkv_mapping(input).reshape(B_, N, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
        )  # 32头注意力，需要是32的倍数，比较困难
        q, k, v = qkv.unbind(0)
        # Scale query
        q = q * self.scale
